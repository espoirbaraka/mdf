...
<?php
  ...
  var options = {
    series: [<?php echo $valide['nbre'];  ?>,<?php echo $pasvalide['nbre'];  ?>],
    legend: {
      show: false,
    },
    dataLabels: {
  ...
  $mesprojets = array();
  $nosnombres = array();
  $data = $_SESSION['CodeUser'];
  $stmt1 = "SELECT id_projet,SUBSTRING(bailleur, 1, 5) as titre FROM tbl_projet ORDER BY id_projet DESC LIMIT 10";
  if($projet = $app->fetchPrepared($stmt1)){
      foreach($projet as $row)
      {
        $projet = $row['titre'];
        $pr = $row['id_projet'];
        $stmt2 = "SELECT budget FROM tbl_projet WHERE id_projet=$pr";
        if($budget = $app->fetchPrepared($stmt2)){
            foreach($budget as $row)
            {
              $montant = $row['budget'];
            }
            array_push($nosnombres, $montant);
        }
        array_push($mesprojets, $projet);
      }

  }

  $projets = json_encode($mesprojets);
  echo $budget = json_encode($nosnombres);
...
?>
  ...
  $nosnombres = array();
  $data = $_SESSION['CodeUser'];
  $stmt1 = "SELECT id_projet,SUBSTRING(bailleur, 1, 15) as titre FROM tbl_projet WHERE addBy=$data";
  if($projet = $app->fetchPrepared($stmt1)){
      foreach($projet as $row)
      {
        $projet = $row['titre'];
...
?>
  ...
  var chart = new ApexCharts(document.querySelector("#user_dashboard"), options); chart.render();





  // Total user dashboard
  var optionsSpark1 = {
    series: [
      {
        name: "Sales",
  ...
  var options = {
    ...
    <?php
    $requette2='SELECT COUNT(id_agent) as nbre from tbl_agent WHERE sexe_agent ="Masculin"';
    $requette3='SELECT COUNT(id_agent) as nbre from tbl_agent WHERE sexe_agent ="Feminin"';
    $homme= $app->fetch($requette2);
    $femme= $app->fetch($requette3);
    ?>
    dataLabels: {
      enabled: false,
    },
    ...
    colors: ['var(--chart-color1)','var(--chart-color2)'],
    series: [<?php echo $homme['nbre'];  ?>,<?php echo $femme['nbre'];  ?>],
